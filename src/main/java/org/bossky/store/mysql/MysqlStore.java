...
import org.bossky.store.db.WhereCondition;
 ...
 * @param <T>
 */
public class MysqlStore<T extends Storeble> extends DbStore<T> {
	/** 真值 */
	...
	protected MysqlStore(MysqlStoreHub hub, Class<T> clazz, Object... initargs) {
	...
	protected ResultPage<T> doSearch(String start, String end) {
		String where = getWhere(start, end);
		return new MysqlResultPage<T>(this, where);
	}

	@Override
	protected ResultPage<T> doQuery(WhereCondition condition) {
		return new MysqlResultPage<T>(this, condition.getWhere());
	}
	 ...
	 * @return
	 */
	protected List<T> search(String where, int startIndex, int endIndex) {
		String sql = searchSql(where, startIndex, endIndex);
		DbExecuter executer = hub.getExecuter();
		ResultSet rs = null;
		try {
	...
	protected String getWhere(String start, String end) {
		StringBuilder sb = new StringBuilder();
	 ...
	 * @return
	 */
	protected String searchSql(String where, int startIndex, int endIndex) {
		List<Meta> metas = mapper.getMetas();
		StringBuilder sb = new StringBuilder();
		sb.append("SELECT ");
		for (Meta m : metas) {
	 ...
	 * @return
	 */
	protected String countSql(String where) {
		StringBuilder sb = new StringBuilder();
		sb.append("SELECT count(`").append(__STORE_ID).append("`) ");
		sb.append(" FROM `");
		sb.append(tableName());
		sb.append("`");
		if (!Misc.isEmpty(where)) {// 有不为空的属性
			sb.append(where);
		}
		return sb.toString();
	}
